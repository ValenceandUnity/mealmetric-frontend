...
import { adaptPTLogHistoryView } from "@/lib/view-models/pt-log-history";

describe("adaptPTLogHistoryView", () => {
  it("flattens workout logs into sorted mobile rows and preserves pagination metadata", () => {
    const view = adaptPTLogHistoryView({
      items: [
        {
          id: "log-older",
    ...
    expect(view.visibleRowsLabel).toBe("3 history rows");
    expect(view.pageWindowLabel).toBe("1-2 of 2");
    expect(view.paginationStateLabel).toBe("Older entries available");
    expect(view.nextOffsetLabel).toBe("Next offset 30");
  ...
  it("returns safe fallbacks when the workout-log payload is missing or invalid", () => {
    ...
    expect(view.visibleRowsLabel).toBe("0 history rows");
    expect(view.pageWindowLabel).toBe("Page window unavailable");
    expect(view.paginationStateLabel).toBe("End reached");
    expect(view.nextOffsetLabel).toBe("No next page");
  });
});
