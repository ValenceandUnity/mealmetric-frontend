...
export function MobileBottomNav({ role, activePath, className }: MobileBottomNavProps) {
  ...
  return (
    <nav
      className={["mobile-bottom-nav", className ?? ""].filter(Boolean).join(" ")}
      aria-label={`${role} primary navigation`}
    >
      {items.map((item) => {
        const isActive = matchesNavItem(resolvedPath, item);
        const itemClasses = [
        ...
        if (item.disabled) {
          return (
            <button
              ...
              type="button"
              className="mobile-bottom-nav__button mobile-focus-ring"
        ...
        return (
          <Link
            key={`${item.label}-${item.href}`}
            href={item.href}
            className={`${itemClasses} mobile-focus-ring`}
